...
class LocalFileStorage:
    ...
    async def get_urls(self, source_name: str) -> List[str]:
        d = self.base_dir / source_name / "urls.json"
        if not d.exists():
            return []
        try:
            with open(d, "r") as f: return json.load(f)
    ...
    async def get_schema(self, source_name: str) -> Dict[str, Any]:
        d = self.base_dir / source_name / "schema.json"
        if not d.exists():
            return {}
        try:
    ...
    async def get_data(self, source_name: str) -> List[Dict[str, Any]]:
        d = self.base_dir / source_name / "courses.json"
        if not d.exists():
            return []
        try:
            with open(d, "r") as f:
