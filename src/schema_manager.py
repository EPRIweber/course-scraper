...
import urllib3
from src.config import SourceConfig, ValidationCheck, config
...
from src.llm_client import LlamaModel, GemmaModel, GPTModel
...
if __name__ == '__main__':
  ...
  print(f"Testing schema generation for {test_source.name} ({test_source.schema_url})")
  import asyncio
  schema, usage = asyncio.run(generate_schema(test_source))
  print(f"LLM usage: {usage} tokens")
  print(json.dumps(schema, indent=2))
  validation: ValidationCheck = asyncio.run(validate_schema(schema, test_source))
  print(f"Schema valid: {validation.valid}")
  if validation.fields_missing:
    print(f"Missing fields: {validation.fields_missing}")
